...
TEST_CASE("Compiler: compound assignment") {
    SlpVM *vm = slp_vm_new(&tc_allocator);

    // Testing +=
    SlpObjFunction *fn1 = compile_source("$x += 5;", vm);
    REQUIRE(fn1 != nullptr);
    SlpChunk *c1 = fn1->chunk;
    CHECK(c1->code[0] == OP_LOAD_GLOBAL); // load $x
    CHECK(c1->code[3] == OP_PUSH_CONST);  // load 5
    CHECK(c1->code[6] == OP_ADD);         // add
    CHECK(c1->code[7] == OP_STORE_GLOBAL);// store $x

    // Testing .=
    SlpObjFunction *fn2 = compile_source("$s .= \"a\";", vm);
    REQUIRE(fn2 != nullptr);
    SlpChunk *c2 = fn2->chunk;
    CHECK(c2->code[0] == OP_LOAD_GLOBAL); // load $s
    CHECK(c2->code[3] == OP_PUSH_CONST);  // load "a"
    CHECK(c2->code[6] == OP_CONCAT);      // concat
    CHECK(c2->code[7] == OP_STORE_GLOBAL);// store $s
...
TEST_CASE("Bytecode: serialize/deserialize chunk") {
    ...
    if (SLP_IS_OBJ(chunk2->constants[0])) {
        tc_allocator.reallocate((void*)SLP_AS_STRING(chunk2->constants[0]), 0, NULL);
    }
    if (SLP_IS_OBJ(chunk.constants[0])) {
        tc_allocator.reallocate((void*)SLP_AS_STRING(chunk.constants[0]), 0, NULL);
    }
    slp_chunk_free(chunk2);
    tc_allocator.reallocate(chunk2, 0, NULL);
...
TEST_CASE("Bytecode: serialize/deserialize function") {
    ...
    slp_bytecode_writer_free(&writer);
    if (fn2) {
        slp_chunk_free(fn2->chunk);
        tc_allocator.reallocate(fn2->chunk, 0, NULL);
        tc_allocator.reallocate(fn2, 0, NULL);
    }
    slp_vm_free(vm);
}
...
TEST_CASE("Bytecode: oversized string length is rejected, not over-read") {
    // Serialize a real function, then corrupt the name_len field (bytes 8..11,
    // right after the 4-byte magic and 4-byte version) to a huge value. The
    // deserializer must reject it instead of allocating/reading out of bounds.
    SlpVM *vm = slp_vm_new(&tc_allocator);
    SlpObjFunction *fn = compile_source("1 + 2;", vm);
    REQUIRE(fn != nullptr);
    ...
    SlpBytecodeWriter writer;
    slp_bytecode_writer_init(&writer, &tc_allocator);
    REQUIRE(slp_bytecode_serialize_function(&writer, fn));
    REQUIRE(writer.count > 12);

    writer.buffer[8] = 0xFF;
    writer.buffer[9] = 0xFF;
    writer.buffer[10] = 0xFF;
    writer.buffer[11] = 0xFF; // name_len = 0xFFFFFFFF
...
TEST_CASE("Compiler: @_ local variable resolution") {
    ...
    // Verify that @_ = $null; in the sub_fn chunk compiles to OP_STORE_LOCAL with slot 11
    SlpChunk *sub_chunk = sub_fn->chunk;
    bool found_store_local_11 = false;
    for (int i = 0; i < sub_chunk->count - 1; i++) {
        if (sub_chunk->code[i] == OP_STORE_LOCAL && sub_chunk->code[i + 1] == 11) {
            found_store_local_11 = true;
            break;
        }
    }
    CHECK(found_store_local_11);

    slp_vm_free(vm);
}

TEST_CASE("Compiler: repl_mode compilation") {
    SlpVM *vm = slp_vm_new(&tc_allocator);
    
    // Test case 1: single expression
    {
        SlpParser parser;
        slp_parser_init(&parser, "1 + 2;", &tc_allocator);
        SlpASTNode *ast = slp_parser_parse(&parser);
        REQUIRE(ast != nullptr);
        
        SlpObjFunction *fn = slp_compile_ex(vm, ast, true, &tc_allocator);
        slp_parser_free_node(ast, &tc_allocator);
        REQUIRE(fn != nullptr);
        
        // In REPL mode, instead of emitting OP_ADD followed by OP_POP,
        // it should emit OP_ADD, then OP_RETURN (leaving the result on the stack for printing).
        SlpChunk *c = fn->chunk;
        
        // Find if OP_POP is emitted. It should NOT be.
        ...
        CHECK_FALSE(found_pop);
        
        // The last instruction should be OP_RETURN.
        CHECK(c->code[c->count - 1] == OP_RETURN);
    }
    ...
    {
        SlpParser parser;
        slp_parser_init(&parser, "1 + 2; 3 + 4;", &tc_allocator);
        SlpASTNode *ast = slp_parser_parse(&parser);
        REQUIRE(ast != nullptr);
        ...
        // The intermediate "1 + 2;" statement should have OP_POP,
        // but the final "3 + 4;" statement should end with OP_RETURN (no OP_POP for the last one).
        SlpChunk *c = fn->chunk;
        
        int pop_count = 0;
        int return_count = 0;
        for (int i = 0; i < c->count; i++) {
            if (c->code[i] == OP_POP) pop_count++;
            if (c->code[i] == OP_RETURN) return_count++;
        }
        
        // Only the first expression should be popped.
        CHECK(pop_count == 1);
        // The compile output ends with OP_RETURN.
...
// ---------------------------------------------------------------------------
// Codegen limit guards (Phase 1: Tier-1 safety)
// ---------------------------------------------------------------------------

TEST_CASE("Compiler: constant-pool overflow errors instead of aliasing") {
    // >65536 distinct constants must fail to compile rather than silently
    // wrap the 16-bit constant index back to 0.
    SlpVM *vm = slp_vm_new(&tc_allocator);
    std::string source;
    source.reserve(500000);
    for (int i = 0; i < 70000; i++) {
        source += std::to_string(i);
        source += ';';
    }
    SlpObjFunction *fn = compile_source(source.c_str(), vm);
    CHECK(fn == nullptr);
    slp_vm_free(vm);
}

TEST_CASE("Compiler: oversized loop body errors instead of truncating offset") {
    // A loop body larger than INT16_MAX bytes of bytecode must error, since
    // OP_LOOP decodes its back-jump operand as a signed 16-bit value.
    SlpVM *vm = slp_vm_new(&tc_allocator);
    std::string source = "while (true) {";
    for (int i = 0; i < 20000; i++) source += "$x = 1;"; // each stmt emits several bytes
    source += "}";
    SlpObjFunction *fn = compile_source(source.c_str(), vm);
    CHECK(fn == nullptr);
    slp_vm_free(vm);
