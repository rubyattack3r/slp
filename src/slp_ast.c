...
char *slp_ast_format(SlpASTNode *node, SlpAllocator *allocator) {
  ...
  char *result = NULL;
  if (buffer.length > 0) {
    result = (char *)allocator->reallocate(NULL, buffer.length + 1, allocator->user_data);
    slp_utils_strcpy(result, buffer.data);
  }
...
SlpASTNode *slp_ast_build_node(SlpASTNodeType type, int line,
                                     SlpAllocator *allocator) {
  SlpASTNode *node =
      (SlpASTNode *)allocator->reallocate(NULL, sizeof(SlpASTNode), allocator->user_data);
  if (!node)
    return NULL;
...
void slp_ast_set_string_val(SlpASTNode *node, const char *str,
  ...
  char *dup = (char *)allocator->reallocate(NULL, len + 1, allocator->user_data);
  if (dup) {
    strcpy(dup, str);
    switch (node->type) {
    case SLP_AST_STRING:
    ...
    case SLP_AST_ADDRESS:
      ...
      break;
    default:
      allocator->reallocate(dup, 0, allocator->user_data);
      break;
    }
  }
}
...
void slp_ast_set_children(SlpASTNode *node, SlpASTNode **children,
  ...
  default:
    if (arr)
      allocator->reallocate(arr, 0, allocator->user_data);
    break;
  }
}
...
void slp_parser_free_node(SlpASTNode *node, SlpAllocator *allocator) {
  ...
  case SLP_AST_ADDRESS:
    if (node->as.string_val)
      allocator->reallocate((void *)node->as.string_val, 0, allocator->user_data);
    break;
  default:
    break;
  }
  ...
  case SLP_AST_LVALUE_TUPLE:
    if (node->as.block.statements) {
      ...
      allocator->reallocate(node->as.block.statements, 0, allocator->user_data);
    }
    break;
  case SLP_AST_CALL:
    if (node->as.call.args) {
      ...
      allocator->reallocate(node->as.call.args, 0, allocator->user_data);
    }
    if (node->as.call.target)
  ...
  case SLP_AST_ENV_BRIDGE:
    ...
    if (node->as.env_bridge.keyword) {
      allocator->reallocate((void *)node->as.env_bridge.keyword, 0, allocator->user_data);
    }
    if (node->as.env_bridge.identifier) {
      allocator->reallocate((void *)node->as.env_bridge.identifier, 0, allocator->user_data);
    }
    if (node->as.env_bridge.string) {
      allocator->reallocate((void *)node->as.env_bridge.string, 0, allocator->user_data);
    }
    break;
  case SLP_AST_BINOP:
  ...
  case SLP_AST_FOR:
    if (node->as.for_stmt.initializer) {
      ...
      allocator->reallocate(node->as.for_stmt.initializer, 0, allocator->user_data);
    }
    if (node->as.for_stmt.condition)
    ...
    if (node->as.for_stmt.increment) {
      ...
      allocator->reallocate(node->as.for_stmt.increment, 0, allocator->user_data);
    }
    if (node->as.for_stmt.body)
  ...
  case SLP_AST_FOREACH:
    ...
    if (node->as.foreach.index)
      allocator->reallocate((void *)node->as.foreach.index, 0, allocator->user_data);
    if (node->as.foreach.value)
      allocator->reallocate((void *)node->as.foreach.value, 0, allocator->user_data);
    break;
  case SLP_AST_ASSIGN_LOOP:
    if (node->as.assign_loop.generator)
      slp_parser_free_node(node->as.assign_loop.generator, allocator);
    if (node->as.assign_loop.body)
      slp_parser_free_node(node->as.assign_loop.body, allocator);
    if (node->as.assign_loop.value)
      allocator->reallocate((void *)node->as.assign_loop.value, 0, allocator->user_data);
    break;
  case SLP_AST_ASSIGNMENT:
  ...
  case SLP_AST_TRY_CATCH:
    ...
    if (node->as.try_catch.value)
      allocator->reallocate((void *)node->as.try_catch.value, 0, allocator->user_data);
    break;
  case SLP_AST_RETURN:
  case SLP_AST_THROW:
  case SLP_AST_ASSERT:
  ...
  case SLP_AST_IMPORT:
    if (node->as.import_stmt.path)
      allocator->reallocate((void *)node->as.import_stmt.path, 0, allocator->user_data);
    if (node->as.import_stmt.target)
      allocator->reallocate((void *)node->as.import_stmt.target, 0, allocator->user_data);
    break;
  case SLP_AST_INDEX:
    if (node->as.index.container)
  ...
  case SLP_AST_OBJ_EXPR:
    ...
    if (node->as.obj_expr.args) {
      ...
      allocator->reallocate(node->as.obj_expr.args, 0, allocator->user_data);
    }
    break;
  default:
    break;
  ...
  allocator->reallocate(node, 0, allocator->user_data);
}
...
static void format_node(SlpASTNode *node, SlpStringBuffer *buffer,
  ...
  case SLP_AST_SCRIPT: {
    for (size_t i = 0; i < node->as.block.count; i++) {
      ...
      SlpASTNodeType t = node->as.block.statements[i]->type;
      if (t != SLP_AST_ENV_BRIDGE && t != SLP_AST_IF && t != SLP_AST_WHILE &&
          t != SLP_AST_FOR && t != SLP_AST_FOREACH && t != SLP_AST_ASSIGN_LOOP &&
          t != SLP_AST_TRY_CATCH) {
  ...
  case SLP_AST_BLOCK: {
    ...
    for (size_t i = 0; i < node->as.block.count; i++) {
      ...
      if (node->as.block.statements[i]->type != SLP_AST_ENV_BRIDGE &&
          ...
          node->as.block.statements[i]->type != SLP_AST_ASSIGN_LOOP &&
          node->as.block.statements[i]->type != SLP_AST_TRY_CATCH) {
        slp_string_buffer_append_char(buffer, ';');
      }
      slp_string_buffer_append_char(buffer, '\n');
    }
  ...
  case SLP_AST_BINOP: {
    PrecedenceLevel parent_prec = get_node_precedence(node);
    
    // Left child formatting
    bool paren_left = (get_node_precedence(node->as.binop.left) < parent_prec);
    if (paren_left) slp_string_buffer_append_char(buffer, '(');
    ...
    // Right child formatting
    bool paren_right = (get_node_precedence(node->as.binop.right) <= parent_prec);
    if (paren_right) slp_string_buffer_append_char(buffer, '(');
  ...
  case SLP_AST_UNARYOP: {
    ...
    if (!is_post) {
      ...
      char first = node->as.unaryop.op.start[0];
      if (node->as.unaryop.op.length > 0 &&
          ((first >= 'a' && first <= 'z') || (first >= 'A' && first <= 'Z') ||
           (first == '-' && node->as.unaryop.op.length > 1 &&
            node->as.unaryop.op.start[1] >= 'a' && node->as.unaryop.op.start[1] <= 'z'))) {
        slp_string_buffer_append_char(buffer, ' ');
      }
    }
  ...
  case SLP_AST_TRY_CATCH: {
    ...
    slp_string_buffer_append_string(buffer, " catch $", 8);
    if (node->as.try_catch.value) {
      slp_string_buffer_append_string(
  ...
  case SLP_AST_IMPORT: {
    ...
    if (node->as.import_stmt.target) {
      slp_string_buffer_append_string(
          ...
          slp_utils_strlen(node->as.import_stmt.target));
    }
    if (node->as.import_stmt.path) {
      slp_string_buffer_append_string(buffer, " from : ", 8);
      slp_string_buffer_append_string(
  ...
  case SLP_AST_CALLCC: {
    slp_string_buffer_append_string(buffer, "callcc", 6);
    if (node->as.control.value) {
      slp_string_buffer_append_char(buffer, ' ');
      format_node(node->as.control.value, buffer, depth);
    }
  ...
  case SLP_AST_ASSIGN_LOOP: {
    slp_string_buffer_append_string(buffer, "while $", 7);
    slp_string_buffer_append_string(buffer, node->as.assign_loop.value, slp_utils_strlen(node->as.assign_loop.value));
    slp_string_buffer_append_string(buffer, " (", 2);
    format_node(node->as.assign_loop.generator, buffer, depth);
    slp_string_buffer_append_string(buffer, ") ", 2);
    format_node(node->as.assign_loop.body, buffer, depth);
    break;
  }
  ...
  case SLP_AST_ADDRESS: {
    char first = node->as.string_val[0];
