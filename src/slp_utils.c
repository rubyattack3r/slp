...
char *slp_utils_strdup(SlpAllocator *allocator, const char *s) {
  if (!s) return NULL;
  size_t len = slp_utils_strlen(s) + 1;
  char *copy = (char *)SLP_MALLOC(allocator, len);
  if (copy) {
    slp_utils_memcpy(copy, s, len);
  }
  return copy;
}
