...
static void compile_node(SlpCompiler *compiler, SlpASTNode *node) {
    ...
    case SLP_AST_KV_PAIR:
        if (node->as.arg.name) {
            if (node->as.arg.name->type == SLP_AST_IDENTIFIER && node->as.arg.name->as.string_val) {
                SlpObjString *str = intern_str(compiler, node->as.arg.name->as.string_val,
                    (uint32_t)slp_utils_strlen(node->as.arg.name->as.string_val));
                emit_byte(compiler, OP_PUSH_CONST, line);
                emit_short(compiler, make_constant(compiler, SLP_OBJ_VAL(str)), line);
            } else {
