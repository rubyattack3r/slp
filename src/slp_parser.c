...
// input (e.g. thousands of "((((" or "if {" levels) would otherwise exhaust
// the C stack and crash; past this we raise a parse error instead. The bound
// must keep BOTH parsing and the matching recursive free (slp_parser_free_node)
// within the stack on sanitizer and small-stack builds. The heaviest path
// (grouping -> expression -> parse_precedence) uses several stack frames per
// level, so the bound is kept conservative; real code never nests this deep.
#define SLP_PARSER_MAX_DEPTH 100
...
static const char *copy_unquoted_lexeme(SlpParser *parser, SlpToken *token) {
    ...
    char *str = (char *)parser->allocator->reallocate(NULL, inner_len + 1, parser->allocator->user_data);
    if (!str) return NULL;
...
static char *duplicate_string(SlpParser *parser, const char *src, size_t len) {
    char *dest = (char *)parser->allocator->reallocate(NULL, len + 1, parser->allocator->user_data);
    if (dest) {
...
static SlpASTNode *obj_expr(SlpParser *parser, SlpASTNode *left,
  ...
  if (match(parser, SLP_TOKEN_COLON)) {
    ...
    if (node->as.obj_expr.arg_count == 0) {
      error(parser, "Expect at least one argument after ':'.");
    }
...
static SlpASTNode *backtick_expr(SlpParser *parser, SlpASTNode *left,
  ...
  (void)canAssign;
  (void)left;
  SlpASTNode *node = allocate_node(parser, SLP_AST_BACKTICK);
  SlpToken t = parser->previous;
  if (t.length > 0) {
    t.start++;
    t.length -= 2; // Remove the surrounding backticks
  }
...
static SlpASTNode *block_expr(SlpParser *parser, SlpASTNode *left,
  ...
  SlpASTNode *node = allocate_node(parser, SLP_AST_ENV_BRIDGE);
  node->as.env_bridge.keyword = NULL;
  node->as.env_bridge.identifier = NULL;
  node->as.env_bridge.string = NULL;

  SlpASTNode *body = allocate_node(parser, SLP_AST_BLOCK);
  body->as.block.capacity = 8;
  body->as.block.count = 0;
  body->as.block.statements = (SlpASTNode **)parser->allocator->reallocate(
      NULL, sizeof(SlpASTNode *) * body->as.block.capacity,
      parser->allocator->user_data);
  while (!check(parser, SLP_TOKEN_RIGHT_BRACE) &&
         !check(parser, SLP_TOKEN_EOF) && !parser->had_error) {
    SlpASTNode *stmt = declaration(parser);
    if (stmt) {
      if (body->as.block.count >= body->as.block.capacity) {
        body->as.block.capacity *= 2;
        body->as.block.statements =
            (SlpASTNode **)parser->allocator->reallocate(
                body->as.block.statements,
                sizeof(SlpASTNode *) * body->as.block.capacity,
                parser->allocator->user_data);
      }
      body->as.block.statements[body->as.block.count++] = stmt;
    }
  }
...
static SlpASTNode *parse_precedence_inner(SlpParser *parser,
                                       ParsePrecedence precedence) {
  advance(parser);
...
static SlpASTNode *parse_precedence(SlpParser *parser,
                                       ParsePrecedence precedence) {
  if (parser->depth >= SLP_PARSER_MAX_DEPTH) {
    error(parser, "Expression nested too deeply.");
    return NULL;
  }
  parser->depth++;
  SlpASTNode *node = parse_precedence_inner(parser, precedence);
  parser->depth--;
  return node;
}
...
static SlpASTNode *while_statement(SlpParser *parser) {
  ...
  if (assign_var) {
    ...
    assign_var->as.string_val = NULL;
    slp_parser_free_node(assign_var, parser->allocator);
...
static SlpASTNode *statement_inner(SlpParser *parser) {
...
static SlpASTNode *statement(SlpParser *parser) {
  if (parser->depth >= SLP_PARSER_MAX_DEPTH) {
    error(parser, "Statements nested too deeply.");
    return NULL;
  }
  parser->depth++;
  SlpASTNode *node = statement_inner(parser);
  parser->depth--;
  return node;
}
