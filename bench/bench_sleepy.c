...
static SleepyAllocator bench_allocator = {bench_alloc, NULL};
...
static double bench_compile(const char *src, int iterations) {
    SleepyVM *vm = sleepy_vm_new(&bench_allocator);
    double start = now_sec();
    for (int i = 0; i < iterations; i++) {
        SleepyParser parser;
        sleepy_parser_init(&parser, src, &bench_allocator);
        SleepyASTNode *ast = sleepy_parser_parse(&parser);
        if (ast) {
            SleepyObjFunction *fn = sleepy_compile(vm, ast, &bench_allocator);
            (void)fn;
            sleepy_parser_free_node(ast, &bench_allocator);
        }
    }
...
int main(void) {
    printf("=== SleepyVM Benchmark Suite ===\n\n");
